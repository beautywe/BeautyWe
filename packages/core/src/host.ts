...
function attachPlugin({ theHost, plugin }) {
  ...
  if (plugin.content.handler) {
    ...
    Object.keys(handlerHooks).forEach((funName) => {
      if (theHost.btPlugin.nativeHookNames.includes(funName)) {
...
class Host {
  ...
  use(plugins) {
    ...
    for (let index = 0; index < plgs.length; index += 1) {
      ...
      if (theHost.btPlugin.plugins.map(item => item.name).includes(plugin.name)) {
      ...
      // record plugin info into this.btPlugin.plugins
      ...
      theHost.btPlugin.plugins.push(pluginInfo);
  ...
  pushHookFun = (funName, func) => {
    this.btPlugin.pluggableFunQueueMap[funName].push(func);
  };

  newHookFunQueue = (funName) => {
    this.btPlugin.pluggableFunQueueMap[funName] = [];
  };

  getHookFunQueue = funName => this.btPlugin.pluggableFunQueueMap[funName];

  pushInitFun = (func) => {
    this.btPlugin.pluginsInitializeQueue.push(func);
  };

  getInitFunQueue = () => this.btPlugin.pluginsInitializeQueue;
}
